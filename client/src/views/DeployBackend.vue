  ...
  <div class="deploy-container">
    <el-card>
      ...
      >
        ...
        <el-form-item label="重启脚本路径" prop="restartScript">
          <el-input 
...
import { deployBackend } from '../api/deploy'
...
const handleDeploy = async () => {
  try {
    ...
    // 执行部署
    const res = await deployBackend(formData)
...
const calculateProgress = (message) => {
  ...
  if (message.includes('执行重启脚本')) return 75
  if (message.includes('文件上传完成')) return 50
  if (message.includes('开始上传文件')) return 25
  if (message.includes('开始部署')) return 10
